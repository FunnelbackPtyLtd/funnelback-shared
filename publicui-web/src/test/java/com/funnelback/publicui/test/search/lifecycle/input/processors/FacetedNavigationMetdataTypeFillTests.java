...
public class FacetedNavigationMetdataTypeFillTests {
	...
	public void testFacetSelected() {
		...
		Assert.assertEquals("|Z:\"$++ australia $++\"", st.getQuestion().getFacetsQueryConstraints().get(1));
		Assert.assertEquals("|Y:\"$++ nsw $++\"", st.getQuestion().getFacetsQueryConstraints().get(0));
