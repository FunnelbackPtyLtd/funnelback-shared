...
jQuery(function() {
	...
	//Need to initiate BS popover & tooltip manually
    ...
    $(document).on('shown.bs.modal load', '#modal-overlay', function() {
            refreshModalContent();
            ...
            if (currentURL.indexOf('sort') === -1) {
                ...
                var href = currentURL.replace(/(sort=)[^\&]+/, '$1' + sortValue).replace(
                    /(&nocache=)[^\&]+/, '');
            }
        ...
        .ready(function() {
			...
			$.each($('#report-details tbody tr'), function(index,value){
				...
				$(this).find('.open-wcag').after(html).on('click',function(){ return false; });
