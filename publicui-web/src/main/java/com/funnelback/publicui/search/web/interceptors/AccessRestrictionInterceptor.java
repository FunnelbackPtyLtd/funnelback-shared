...
public class AccessRestrictionInterceptor implements HandlerInterceptor {
    ...
    public String getConnectingIp(HttpServletRequest request, ServiceConfigReadOnly serviceConfig) {
        String possiblyPseudonomisedIp = request.getRemoteAddr();
        log.trace("Real request IP (ignoring X-Forwarded-For, possibly pseudonomised for logging): " + possiblyPseudonomisedIp);
        String ip = (String) request.getAttribute(IpPseudonomysationFilter.UNPSEUDONOMYSED_IP_ADDRESS_ATTRIBUTE_NAME);
        ...
        log.trace("Connecting IP (pseudonomised for logging) is: " + IpPseudonomysationFilter.pseudonomyseRemoteAddress(ip));
        return ip;
    }
