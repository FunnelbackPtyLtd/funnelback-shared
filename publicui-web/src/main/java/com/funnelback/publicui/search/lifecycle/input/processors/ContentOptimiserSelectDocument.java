...
public class ContentOptimiserSelectDocument implements InputProcessor {
	...
	public void processInput(SearchTransaction searchTransaction)
		...
		if (SearchTransactionUtils.hasCollection(searchTransaction)
				...
				&& searchTransaction.getQuestion().getInputParameterMap().containsKey(RequestParameters.CONTENT_OPTIMISER_URL)
				&& ! searchTransaction.getQuestion().isExtraSearch()) {
			Map<String,String> m = new HashMap<String,String>();
			log.info("Beginning extra search for content optimiser: " +  searchTransaction.getQuestion().getInputParameterMap().get(RequestParameters.CONTENT_OPTIMISER_URL ) );
			m.put(RequestParameters.CONTENT_OPTIMISER_URL, searchTransaction.getQuestion().getInputParameterMap().get(RequestParameters.CONTENT_OPTIMISER_URL ) );
