...
public class CompareConversion implements Operation {
    ...
    public String process(String in) {
        ...
        if (out.contains("<s:Compare")) {
            ...
            if (m.find()) {
                out = m.replaceAll("<#if question.collection.configuration.value(\"$1\")?exists && question.collection.configuration.value(\"$1\") $2 $3>$4</#if>");
            }
            ...
            m = Pattern.compile("<s:Compare\\s+<@s.cfg>\\s*([^>]*)\\s*</@s.cfg>\\s+=~\\s+([^>]+?)\\s*>(.*?)</s:Compare>", Pattern.MULTILINE | Pattern.DOTALL).matcher(out);
            if (m.find()) {
                out = m.replaceAll("<#if question.collection.configuration.value(\"$1\")?exists && question.collection.configuration.value(\"$1\")?matches(\"$2\", \"r\")>$3</#if>");
            }
            m = Pattern.compile("<s:Compare\\s+<@s.cfg>\\s*([^>]*)\\s*</@s.cfg>\\s+!~\\s+([^>]+?)\\s*>(.*?)</s:Compare>", Pattern.MULTILINE | Pattern.DOTALL).matcher(out);
            if (m.find()) {
                out = m.replaceAll("<#if question.collection.configuration.value(\"$1\")?exists && ! question.collection.configuration.value(\"$1\")?matches(\"$2\", \"r\")>$3</#if>");
