...
<#if response.resultPacket.resultsSummary.totalMatching != 0>       
<!-- CSV DOWNLOAD -->
<a class="btn" href="/s/search.html?${QueryString?replace("form=documents","form=csv_export")}&num_ranks=10000">Download as CSV (Maximum 10K Records)</a>          
</#if>
    
    <!-- START RESULTS -->
    <#-- Hide the table if there are no results -->
    <table class="table table-striped <#if response.resultPacket.resultsSummary.totalMatching == 0>hidden</#if>">
            ...
            <tr>
                ...
                <td>
                  ...
                  <form action="${httpRequest.requestURL}/../../content-optimiser/optimise.html">
                    ...
                    <input type="text" name="query" value="${question.inputParameterMap["query"]!?html}" />
