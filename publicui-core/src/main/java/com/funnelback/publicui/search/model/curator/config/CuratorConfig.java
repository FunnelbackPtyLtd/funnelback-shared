...
public class CuratorConfig {
    ...
    /**
     * A map describing the sets of actions to be performed when a request mathcing the given trigger is performed.
     */
    @Getter @Setter
    ...
    public static boolean isCuratorActive(SearchTransaction searchTransaction) {
        return (!searchTransaction.getQuestion().getInputParameterMap().containsKey(RequestParameters.CURATOR)) ||
            Boolean.parseBoolean(searchTransaction.getQuestion().getInputParameterMap().get(RequestParameters.CURATOR));
    }
}
